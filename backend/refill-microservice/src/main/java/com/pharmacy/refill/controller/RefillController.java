...
public class RefillController {
    ...
    @Operation(summary = "Get latest refill status", 
               description = "Retrieve the latest refill order status for the authenticated member")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", 
                    description = "Successfully retrieved refill status",
                    content = @Content(mediaType = "application/json", 
                                     schema = @Schema(implementation = RefillStatusResponse.class))),
        @ApiResponse(responseCode = "404", 
                    description = "No refill orders found",
                    content = @Content(mediaType = "application/json")),
    ...
    @Operation(summary = "Get refill dues as of date", 
               description = "Retrieve all subscriptions that are due for refill as of the specified date")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", 
                    description = "Successfully retrieved refill dues",
                    content = @Content(mediaType = "application/json")),
    ...
    public ResponseEntity<?> getRefillDuesAsOfDate(
            ...
            @Parameter(description = "Due date in YYYY-MM-DD format", example = "2024-01-15")
    ...
    @Operation(summary = "Request adhoc refill", 
               description = "Create a new adhoc refill order for the specified prescriptions")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", 
                    description = "Successfully created adhoc refill order",
                    content = @Content(mediaType = "application/json", 
                                     schema = @Schema(implementation = AdhocRefillResponse.class))),
        @ApiResponse(responseCode = "400", 
        ...
        try {
            ...
            if (!tokenMemberId.equals(request.getMemberId())) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Map.of("error", "Access Denied", "message", "Cannot create refill order for different member"));
            }
    ...
    @GetMapping("/orders")
    @Operation(summary = "Get all refill orders", 
               description = "Retrieve all refill orders for the authenticated member")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", 
                    description = "Successfully retrieved refill orders",
                    content = @Content(mediaType = "application/json")),
        @ApiResponse(responseCode = "401", 
                    description = "Unauthorized access",
                    content = @Content(mediaType = "application/json"))
    ...
    @Operation(summary = "Update refill order status", 
               description = "Update the status of a refill order (Admin operation)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", 
                    ...
                    content = @Content(mediaType = "application/json", 
                                     schema = @Schema(implementation = RefillStatusResponse.class))),
        @ApiResponse(responseCode = "404", 
                    description = "Refill order not found",
                    content = @Content(mediaType = "application/json")),
        @ApiResponse(responseCode = "401", 
                    description = "Unauthorized access",
                    content = @Content(mediaType = "application/json"))
